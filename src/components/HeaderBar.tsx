...
import { Search, Bell, Sun, Moon, ChevronDown, Star, Command, Sparkles, X, CheckSquare, Trash2, ArrowUpRight } from "lucide-react";
...
export default function HeaderBar() {
  ...
  return (
    <header className={`w-full h-18 px-6 flex items-center justify-between border-b ${theme === "light" ? "bg-white border-slate-200 text-slate-800" : "border-slate-800/60 bg-slate-950/20 text-white"} backdrop-blur-md sticky top-0 z-40 font-sans`}>
      
      {/* Tickers Feed Panel */}
      <div className="flex items-center space-x-3.5 overflow-x-auto scrollbar-none select-none flex-1 min-w-0 pr-4 py-1">
        <button className="p-2 rounded-xl text-slate-500 hover:text-amber-400 bg-white/5 hover:bg-white/10 transition-colors border border-white/5 cursor-pointer shrink-0">
          <Star className="h-3.5 w-3.5 fill-current text-slate-500 hover:text-amber-400" />
        </button>

        {/* BTC/USDT Ticker */}
        <div 
          onClick={() => { setActiveTab("dashboard"); setSelectedAsset("BTC"); }} 
          className="px-3 py-1.5 rounded-xl border border-white/5 bg-slate-900/40 hover:border-blue-500/20 hover:bg-slate-900/60 transition-all flex items-center space-x-2.5 cursor-pointer shrink-0"
        >
          <div className="w-4.5 h-4.5 rounded-full bg-amber-500 text-[10px] text-slate-950 font-black flex items-center justify-center font-mono">
            ₿
          </div>
          <div>
            <div className="flex items-center space-x-1 leading-none">
              <span className="text-[10px] font-bold text-white tracking-tight uppercase">BTC</span>
            </div>
            <div className="flex items-center space-x-1.5 mt-0.5 leading-none">
              <span className="text-xs font-mono font-bold text-slate-200">
                {formatCryptoPrice(btcPrice)}
              </span>
              <span className={`text-[9px] font-mono font-bold ${btcChangePercent >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
        ...
        <div 
          ...
          className="px-3 py-1.5 rounded-xl border border-white/5 bg-slate-900/40 hover:border-blue-500/20 hover:bg-slate-900/60 transition-all flex items-center space-x-2.5 cursor-pointer shrink-0"
        >
          <div className="w-4.5 h-4.5 rounded-full bg-indigo-500 text-[10px] text-white font-bold flex items-center justify-center font-mono select-none">
            ♦
          </div>
          <div>
            <div className="flex items-center space-x-1 leading-none">
              <span className="text-[10px] font-bold text-white tracking-tight uppercase">ETH</span>
            </div>
            <div className="flex items-center space-x-1.5 mt-0.5 leading-none">
              <span className="text-xs font-mono font-bold text-slate-200 font-bold">
                {formatCryptoPrice(ethPrice)}
              </span>
              <span className={`text-[9px] font-mono font-bold ${ethChangePercent >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
        ...
        <div 
          ...
          className="px-3 py-1.5 rounded-xl border border-white/5 bg-slate-900/40 hover:border-blue-500/20 hover:bg-slate-900/60 transition-all flex items-center space-x-2.5 cursor-pointer shrink-0"
        >
          <div className="w-4.5 h-4.5 rounded-full bg-cyan-400 text-[8px] text-slate-950 font-black flex items-center justify-center font-mono select-none">
            S
          </div>
          <div>
            <div className="flex items-center space-x-1 leading-none">
              <span className="text-[10px] font-bold text-white tracking-tight uppercase">SOL</span>
            </div>
            <div className="flex items-center space-x-1.5 mt-0.5 leading-none">
              <span className="text-xs font-mono font-bold text-slate-200">
                {formatCryptoPrice(solPrice)}
              </span>
              <span className={`text-[9px] font-mono font-bold ${solChangePercent >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
        ...
        {allQuotes?.["AAPL"] && (
          <div 
            ...
            className="hidden md:flex px-3 py-1.5 rounded-xl border border-white/5 bg-slate-900/40 hover:border-blue-500/20 hover:bg-slate-900/60 transition-all items-center space-x-2.5 cursor-pointer shrink-0"
          >
            <div className="w-4.5 h-4.5 rounded-full bg-sky-500 text-[9px] text-white font-bold flex items-center justify-center font-mono select-none">
              
            </div>
            <div>
              <div className="flex items-center space-x-1 leading-none">
                <span className="text-[10px] font-bold text-white tracking-tight uppercase">AAPL</span>
              </div>
              <div className="flex items-center space-x-1.5 mt-0.5 leading-none">
                <span className="text-xs font-mono font-bold text-slate-200">
                  ${allQuotes["AAPL"].price.toFixed(2)}
                </span>
                <span className={`text-[9px] font-mono font-bold ${allQuotes["AAPL"].changePercent >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
        ...
        {allQuotes?.["S&P 500"] && (
          <div 
            onClick={() => { setActiveTab("stocks-indexes"); }}
            className="hidden lg:flex px-3 py-1.5 rounded-xl border border-white/5 bg-slate-900/40 hover:border-blue-500/20 hover:bg-slate-900/60 transition-all items-center space-x-2.5 cursor-pointer shrink-0"
          >
            <div className="w-4.5 h-4.5 rounded-full bg-purple-500 text-[7.5px] text-white font-black flex items-center justify-center font-mono select-none">
              SPX
            </div>
            <div>
              <div className="flex items-center space-x-1 leading-none">
                <span className="text-[10px] font-bold text-white tracking-tight uppercase">S&P 500</span>
              </div>
              <div className="flex items-center space-x-1.5 mt-0.5 leading-none">
                <span className="text-xs font-mono font-bold text-slate-200">
                  {allQuotes["S&P 500"].price.toFixed(1)}
                </span>
                <span className={`text-[9px] font-mono font-bold ${allQuotes["S&P 500"].changePercent >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
      ...
      <div ref={searchContainerRef} className="relative max-w-[280px] w-full mx-6">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search markets, pairs, bots... [/]"
            className="w-full bg-slate-900/60 hover:bg-slate-900 border border-slate-800 focus:border-blue-500/50 rounded-xl pl-9.5 pr-10 py-2.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none transition-all font-sans"
          />
          <div className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center space-x-0.5 opacity-60 text-[9px] font-mono bg-slate-950 border border-white/5 rounded px-1.5 py-0.5">
      ...
      <div className="flex items-center space-x-4 shrink-0">
        ...
        <div ref={notificationContainerRef} className="relative">
          ...
          {showNotificationDropdown && (
            <div className="absolute top-12 right-0 w-80 py-2 rounded-2xl border border-slate-800/90 bg-slate-950/95 backdrop-blur-xl shadow-[0_12px_40px_rgba(0,0,0,0.85)] z-50">
        ...
        <div ref={profileContainerRef} className="relative">
          ...
          >
            ...
            <ChevronDown className="h-3.5 w-3.5 text-slate-500 group-hover:text-slate-400 transition-colors" />
          </div>
